...
# Matrix Creation
create_variable_matrix <- function(levels, pattern, total_length) {
  # Input:
  # levels: number of levels
  # pattern: pattern of level allocation
  # total_length: total number of trial (state variable) or arm (arm variable)
  # Output:
  # matrix: one-hot encoding matrix, with shape total_length x levels. 
  #         One represents for that trial/arm, this variable takes this level.
  matrix <- matrix(0, nrow = total_length, ncol = levels)
  values <- switch(pattern,
    "Loop" = {
      rep(1:levels, length.out = total_length)
    },
    "Shuffle" = {
      values <- rep(1:levels, length.out = total_length)
      sample(values, length(values), replace = FALSE)
    },
    "Random" = {
      sample(1:levels, total_length, replace = TRUE)
    }
  )
